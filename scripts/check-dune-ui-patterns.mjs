...
for (const [name, source] of [["add form", addFormSource], ["edit form", editFormSource]]) {
  ...
  assert.match(source, /group grid w-full max-w-60 gap-1\.5/, `${name}: tracked-total steppers should reserve enough width for attached actions`)
  assert.match(source, /stepperStateLabel[\s\S]*?rounded-full[\s\S]*?whitespace-nowrap/, `${name}: reconciliation state should render as a compact non-wrapping pill`)
  assert.match(source, /Use tracked[\s\S]*?whitespace-nowrap|whitespace-nowrap[\s\S]*?Use tracked/, `${name}: tracked-total action should stay on one line`)
